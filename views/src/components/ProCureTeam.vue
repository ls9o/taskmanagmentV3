<template>
  <div class="container-fluid">
    ...
    <Paginator :first="(currentPage - 1) * itemsPerPage" :rows="itemsPerPage" :totalRecords="Tasks.length"
      @page="onPageChangeDataInfo" currentPageReportTemplate="Showing {first} to {last} of {totalRecords}"
      class="mb-3" />
    <div class="row">
      <div class="col-12">
...
const fetchTask = async () => {
  try {
    ...
    Tasks.value = response.data.filter(item => {
      return item.infotype === 'จัดซื้อจัดจ้าง' && item.userandteam.includes(userFullName.value);
    });
