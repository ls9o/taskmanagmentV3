<template>
  <div class="container-fluid">
    ...
    <Paginator :first="(currentPage - 1) * itemsPerPage" :rows="itemsPerPage" :totalRecords="Tasks.length"
            @page="onPageChangeDataInfo" class="mb-3"
...
// แก้ไข fetchTask เพื่อรองรับการเรียกข้อมูลตามหน้าที่เลือก
const fetchTask = async () => {
  try {
    const response = await apiService.getTask(); // เพิ่ม page, itemsPerPage ถ้าจำเป็น
    Tasks.value = response.data.filter(item => item.infotype === "ภายใน");
    Tasks.value = Tasks.value.filter(item => item.create_by === userStorage.value.userID);
